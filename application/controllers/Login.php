...
class Login extends CI_Controller {
    ...
    public function index() {
		...
		//echo "here is me".$this->aauth->is_admin();
		//exit();		
        /* redirects if already loggined */
        if($this->aauth->is_loggedin() && $this->aauth->is_admin() == '') 
		{
            //echo "here ". $this->aauth->is_admin();
			redirect('campaignlist', 'refresh');
        }
		else if($this->aauth->is_loggedin() && $this->aauth->is_admin() == 1) 
		{
    ...
    function create_user($user, $pwd, $role) {

        $a = $this->aauth->create_user($user, $pwd, $role);
